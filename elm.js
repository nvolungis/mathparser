...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var metricView = function (metric) {
      return A2($Html.li,
      _L.fromArray([]),
      _L.fromArray([$Html.text($Basics.toString(metric.count))]));
   };
   var metricsView = function (metrics) {
      return function () {
   ...
   var titleView = A2($Html.h1,
   _L.fromArray([]),
   _L.fromArray([$Html.text("Configurable Metrics")]));
   var equationString = _L.fromArray(["1"
                                     ,"+"
                                     ,"2"]);
   var nullMetric = {_: {}
                    ,count: 0
   ...
   var initialState = {_: {}
                      ,metrics: _L.fromArray([{_: {}
                                             ...
                                             ,{_: {}
                                              ...
                                              ,id: 2
                                              ,title: "Metric 2"}])};
   var getMetric = function ($int) {
      return function () {
         var maybeMetric = $List.head(A2($List.filter,
         function (metric) {
            return _U.eq(metric.id,$int);
         },
         initialState.metrics));
         return function () {
            switch (maybeMetric.ctor)
            {case "Just":
               return maybeMetric._0;
               case "Nothing":
               return nullMetric;}
            _U.badCase($moduleName,
   ...
   var getMetricTitle = function ($int) {
      return getMetric($int).title;
   };
   var getMetricCount = function ($int) {
      return getMetric($int).count;
   };
   var maybeMatch = function (str) {
      return function () {
         var _v2 = $String.toInt(str);
         switch (_v2.ctor)
         {case "Err":
            return A2($Basics._op["++"],
              ...
              A2($Basics._op["++"],str," "));
            case "Ok":
            return $Basics.toString(getMetricCount(_v2._0));}
         _U.badCase($moduleName,
         "between lines 60 and 65");
      }();
   };
   var interpolateVal = function (str) {
      return maybeMatch(str);
   };
   var interpolateEquationVals = function (equation) {
      return A3($List.foldl,
      F2(function (val,$final) {
         return A2($Basics._op["++"],
         $final,
         interpolateVal(val));
      }),
   ...
   var equationView = function (metrics) {
      return function () {
         var vals = interpolateEquationVals(equationString);
         return A2($Html.h1,
         _L.fromArray([]),
         _L.fromArray([$Html.text(vals)]));
      }();
   };
   var view = function (state) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([titleView
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,interpolateEquationVals: interpolateEquationVals
                      ,interpolateVal: interpolateVal
                      ,maybeMatch: maybeMatch
                      ,getMetricTitle: getMetricTitle
